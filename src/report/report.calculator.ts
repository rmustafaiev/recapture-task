...
export class Calculator implements Calculator {
  ...
  getTopProducts(): Array<Product> {
    const products = Array.from(this.topProducts.entries()).sort(
      (a, b) => b[1] - a[1],
    );
...
export class CartCalculator extends Calculator {
  ...
  result(): CartReport {
    const rate: number = Math.round(
      (this.abandonedCarts / this.totalCarts) * 100,
    );
...
export class OrderCalculator extends Calculator {
  ...
  result(): OrderReport {
    ...
    const top5 = this.getTopProducts();
    const sales: Array<SalesPerDay> = Array.from(this.salesPerDay.entries())
